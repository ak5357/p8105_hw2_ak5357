...
Based on the outputs of the code chunk above, we can see that `bakers_df` does not have any `NA` values, `bakes_df` has `NA`s in the `show_stopper` column, and `results_df` has hundreds of `NA`s throughout the `technical` and `results` columns




```{r qaqc_bakes_results, eval = FALSE}
