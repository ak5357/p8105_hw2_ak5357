...
There are still some rows with missing data. There are two sources for
these `NA` values.
...
my control, so I don’t see a point in going down that road. As such,
let’s move on.*
...
That was a long process. In my final code, I would have done it like so:
